...
//            }
    ...
    final int scanSimpleName(int bufAt, String s) throws ParseException
    {
        ...
        if (lookahead < 256)
        {
            ...
            String sharepoint = System.getProperty( "SharePoint", "no" );
            if( sharepoint.equalsIgnoreCase("yes") || sharepoint.equalsIgnoreCase( "true" ) )
        ...
        for (;;)
        {
            ...
            switch(token)
            {
                case TAGSTART:
                    ...
                    {
                        if (ed != null && !ed.acceptEmpty())
                        {
                            reportMismatch(0, name.getName() + " cannot be empty.");
                        }
...
//        current.checkAllowText();
    ...
    final Element parseNameSpaceDecl(boolean global, boolean PI) throws ParseException
    {
        ...
        if (caseInsensitive)
        {
            ...
            href = Atom.create(hrefStr.toString().toUpperCase());
        }
        else
        {
            as = Atom.create(asStr.toString());
            href = Atom.create(hrefStr.toString());
        }
        if (DTD.isReservedNameSpace(as))
        {
            error(as.toString() + " is a reserved name space.");
        ...
        if (loadexternal) { // load name space
