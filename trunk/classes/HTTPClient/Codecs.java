...
public class Codecs
{
    ...
    private final static String Boundary = "\r\n-----ieoau=_=_+2:8?GoodLuck8)3(dskdfJwSJKlrWLr0234324jfLdsjfdAuaoei-----";
    ...
    static
    {
	// rfc-2046: bcharsnospace - used for multipart codings
	...
	{
	    qbuf.append(URLEncode(pairs[idx].name) + "=" +
			URLEncode(pairs[idx].value) + "&");
	}
	...
	if (last)
	{
	    ...
	    for (int idx=0; idx<ftrs.length; idx++)
	    {
		ftrs[idx].name.getBytes(0, ftrs[idx].name.length(), res, r_off);
		r_off += ftrs[idx].name.length();

		res[r_off++] = (byte) ':';
		res[r_off++] = (byte) ' ';

		ftrs[idx].value.getBytes(0, ftrs[idx].value.length(), res, r_off);
		r_off += ftrs[idx].value.length();
