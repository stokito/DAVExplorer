/*
 * @(#)HTTPConnection.java				0.3 30/01/1998
 *
	...
	implements GlobalConstants, HTTPClientModuleConstants
{
    ...
    public final static String   version = "RPT-HTTPClient/0.3";
	...
	if (Port == -1)
	    Port = Util.defaultPort(getProtocol());
	...
	for (int idx=0; idx<non_proxy_dom_list.size(); idx++)
	    if (host.toLowerCase().endsWith((String) non_proxy_dom_list.elementAt(idx)))
		return true;
    ...
    {
	return DefaultModuleList.removeElement(module);
    }
	...
	if (host.charAt(0) == '.')
	{
	    if (!non_proxy_dom_list.contains(host.toLowerCase()))
		non_proxy_dom_list.addElement(host.toLowerCase());
	    return;
	}
	...
	if (host.charAt(0) == '.')
	    return non_proxy_dom_list.removeElement(host.toLowerCase());
	...
	try
	{
	    HTTPResponse resp = new HTTPResponse(gen_mod_insts(), Timeout);
     ...
     */
    void handleRequest(Request req, HTTPResponse resp, boolean usemodules)
		throws IOException, ModuleException
    {
	Response[]         rsp_arr = null;
	HTTPClientModule[] modules = resp.getModules();
	...
	doModules: for (int idx=0; idx<modules.length; idx++)
	{
		...
		case REQ_NEWCON_RST:	// new connection
		    if (req.internal_subrequest)  return;
		    req.getConnection().handleRequest(req, resp, true);
		    return;

		case REQ_NEWCON_SND:	// new connection, send immediately
		    if (req.internal_subrequest)  return;
		    req.getConnection().handleRequest(req, resp, false);
		    return;
	...
	if (req.getStream() != null  &&  req.getStream().getLength() == -1)
	{
	    ...
	    {
		req.getStream().goAhead(req, null, resp.getTimeout());
		resp.set(req, req.getStream());
	    }
	    else
	    {
		...
		resp.set(req, sendRequest(req, resp.getTimeout()));
	    }
	}
	else
	    resp.set(req, sendRequest(req, resp.getTimeout()));
    ...
    Response sendRequest(Request req, int con_timeout)
		throws IOException, ModuleException
    {
	ByteArrayOutputStream hdr_buf = new ByteArrayOutputStream(500);
	Response              resp = null;
	boolean		      keep_alive;
	...
	// get a new response. Note: this does not do a read on the socket.

	if (resp == null)
	    resp = new Response(req, (Proxy_Host != null && Protocol != HTTPS),
				input_demux);
	...
	else if (!IdempotentSequence.methodIsIdempotent(req.getMethod()))
	    { late_stall = resp; }


	/* I've just seen problems with Apache 1.2.5 and pipelining. This
	 * is a hack to be able to prevent all pipelining:
	 */
	if (NeverPipeline)
	    { late_stall = resp; }
	...
	{
	    String name = req.getHeaders()[idx].getName().toLowerCase();
	...
	ByteArrayOutputStream hdr_buf = new ByteArrayOutputStream(500);
	HTTPResponse r = new HTTPResponse(gen_mod_insts(), timeout);
	...
	while (true)
	{
	    ...
	    Response resp = new Response(connect, sock[0].getInputStream());
     ...
     *                     if the "chunked" token is not specified this token
	...
	{
	    String name = hdrs[idx].name.trim();
	    if (name.equalsIgnoreCase("Content-type"))     ct_idx = idx;
	    if (name.equalsIgnoreCase("User-Agent"))       ua_idx = idx;
	    if (name.equalsIgnoreCase("Connection"))       co_idx = idx;
	    if (name.equalsIgnoreCase("Proxy-Connection")) pc_idx = idx;
	    if (name.equalsIgnoreCase("Keep-Alive"))       ka_idx = idx;
	    if (name.equalsIgnoreCase("Expect"))           ex_idx = idx;
	    if (name.equalsIgnoreCase("TE"))               te_idx = idx;
	}
	      ...
	      co_idx == -1))
	{
	    ...
	    {
		con_hdrs[0] = hdrs[co_idx].value.trim();
		co_hdr = con_hdrs[0];
	    }
	...
	if (te_idx != -1)
	{
	    ...
	    Vector pte;
	    try
		{ pte = Util.parseHeader(hdrs[te_idx].value); }
	    catch (ParseException pe)
		{ throw new IOException(pe.toString()); }

	    if (!pte.contains(new HttpHeaderElement("chunked")))
		dataout.writeBytes("chunked, ");

	    dataout.writeBytes(hdrs[te_idx].value.trim() + "\r\n");
	}
	else
	    dataout.writeBytes("TE: chunked\r\n");


	// User-Agent

	if (ua_idx != -1)
	    dataout.writeBytes("User-Agent: " + hdrs[ua_idx].value.trim() + " "
			       + version + "\r\n");
	else
	...
	for (int idx=0; idx<hdrs.length; idx++)
	{
		...
		idx != te_idx)
		dataout.writeBytes(hdrs[idx].name.trim() + ": " +
				   hdrs[idx].value.trim() + "\r\n");
	}
	...
	if (req.getData() != null  ||  req.getStream() != null)
	{
	    ...
	    if (ct_idx != -1)
		dataout.writeBytes(hdrs[ct_idx].value.trim());
	    else
		...
		dataout.writeBytes("Content-length: " +req.getData().length +
				   "\r\n");
	    else if (req.getStream().getLength() != -1)
		dataout.writeBytes("Content-length: " +
				   req.getStream().getLength() + "\r\n");

	    if (ex_idx != -1)
	    {
		con_hdrs[1] = hdrs[ex_idx].value.trim();
	...
	else if (ex_idx != -1)
	{
	    ...
	    try
		{ expect_tokens = Util.parseHeader(hdrs[ex_idx].value); }
	    catch (ParseException pe)
		{ throw new IOException(pe.toString()); }
    ...
    {
	if (input_demux != null)  input_demux.close(ioe, true);
	...
	return getProtocol() + "://" + getHost() +
	    (getPort() != Util.defaultPort(getProtocol()) ? ":" + getPort() : "");
    }
}


class EstablishConnection extends Thread
{
    String      actual_host;
    int         actual_port;
